// Menel Invaders – v6
// • Adds A/D keys for movement
// • Improved procedural zig-zag crack lines when crack images missing
// • Beer HUD moved to very top and gameplay area shifted downward
...
class MenelInvaders {
  constructor() {
    ...
    /* Layout */
    this.hudHeight = 60; // reserve pixels at very top
    ...
    /* Crack imgs */
    this.crackImgs = [1, 2, 3].map(n => {
      const i = new Image();
      i.src = `assets/crack${n}.png`;
      return i;
    });
    ...
    this.enemyOffsetY = this.hudHeight + 20; // leave HUD area clear
  ...
  drawStars() {
    ...
    this.stars.forEach(s => this.ctx.fillRect(s.x, s.y, 2, 2));
  }
  ...
  setupControls() {
    window.addEventListener('keydown', e => {
      if (['ArrowLeft', 'a', 'A'].includes(e.key)) this.leftPressed = true;
      if (['ArrowRight', 'd', 'D'].includes(e.key)) this.rightPressed = true;
      if (e.key === ' ') {
    ...
    window.addEventListener('keyup', e => {
      if (['ArrowLeft', 'a', 'A'].includes(e.key)) this.leftPressed = false;
      if (['ArrowRight', 'd', 'D'].includes(e.key)) this.rightPressed = false;
    });
  }
  ...
  update() {
    ...
    this.enemies.forEach(e => {
      ...
      if (e.x <= 0 || e.x + e.width >= this.canvas.width) needReverse = true;
    });
    if (needReverse) this.enemyDirection *= -1;
    ...
    this.playerBullets.forEach((b, bi) => {
      this.enemies.forEach(e => {
        ...
        if (b.x < e.x + e.width && b.x + b.width > e.x && b.y < e.y + e.height && b.y + b.height > e.y) {
          ...
          if (e.hp <= 0) {
            e.alive = false;
            this.score += this.alienTypes[e.type].score;
            this.playSound('explosion');
          }
        }
      });
  ...
  /* ---------------- Draw helpers ---------------- */
  drawZigZagCrack(x, y, w, h) {
    const segments = 6;
    const stepX = w / segments;
    let curX = x;
    let curY = y + Math.random() * h * 0.3;
    this.ctx.beginPath();
    this.ctx.moveTo(curX, curY);
    for (let i = 0; i < segments; i++) {
      curX += stepX;
      curY += (Math.random() - 0.5) * h * 0.3; // random vertical shift
  ...
  drawCracks(x, y, w, h, severity) {
    severity = Math.min(severity, this.crackImgs.length);
    const useImg = this.crackImgs[0].complete && this.crackImgs[0].naturalWidth;
    if (useImg) {
      for (let i = 0; i < severity; i++) this.ctx.drawImage(this.crackImgs[i], x, y, w, h);
    } else {
      this.ctx.strokeStyle = 'rgba(255,255,255,0.4)';
      this.ctx.lineWidth = 2;
      for (let i = 0; i < severity; i++) this.drawZigZagCrack(x, y, w, h);
    }
  }
  ...
  draw(ts) {
    ...
    // Player
    if (this.playerImg.complete && this.playerImg.naturalWidth) this.ctx.drawImage(this.playerImg, this.player.x, this.player.y, this.player.width, this.player.height);
    else {
    ...
    this.enemies.forEach(e => {
      if (!e.alive) return;
      const yRender = e.y + Math.sin(time / 300 + e.oscPhase) * 3;
      const info = this.alienTypes[e.type];
      const img = info.imageObj;
      if (img.complete && img.naturalWidth) {
        const ratio = img.naturalWidth / img.naturalHeight;
        let dw = e.width, dh = e.height;
        if (ratio > 1) dh = dw / ratio; else dw = dh * ratio;
        this.ctx.drawImage(img, e.x + (e.width - dw) / 2, yRender + (e.height - dh) / 2, dw, dh);
      } else {
        this.ctx.fillStyle = ['#ff6666', '#ff3333', '#cc0000'][e.type] || '#f00';
        this.ctx.fillRect(e.x, yRender, e.width, e.height);
      }
      if (e.hp < e.maxHp) this.drawCracks(e.x, yRender, e.width, e.height, e.maxHp - e.hp);
    });
  ...
  drawHud() {
    ...
    this.ctx.fillStyle = '#d48b07';
    this.ctx.fillRect(0, hudY, hudW, this.hudHeight);
    this.ctx.fillStyle = '#ffffff';
    this.ctx.fillRect(0, hudY, hudW, 10); // white foam
    this.ctx.fillStyle = '#ffffff';
    this.ctx.font = '18px "Comic Sans MS", Arial';
    this.ctx.fillText('🍺 Score: ' + this.score, 10, hudY + 30);
    this.ctx.fillText('🍺 Lives: ' + this.player.lives, 10, hudY + 50);
    this.ctx.fillText('Lvl: ' + this.level, 200, hudY + 50);
  }
