// Menel Invaders – v9
// • Miniature city photo in start menu (bottom center)
// • Miniature city photo in HUD during gameplay (right side)
// • ESC key returns to main menu from playing/gameover
// Derived from v8
...
class MenelInvaders {
  constructor() {
    ...
    /* Load first level for bg etc */
    this.loadLevel(0);

    requestAnimationFrame(ts=>this.loop(ts));
  }

  /* ------------ Utility image ready check ------------- */
  ...
  /* ------------ Level loading and enemy patterns (reuse v8) ------------- */
  loadLevel(idx){
  ...
  initPattern(cfg){
    this.enemies=[];
    const p=cfg.pattern;
    if(p==='grid') this.placeGrid(cfg.rows,cfg.cols);
    else if(p==='triangle') this.placeTriangle(cfg.rows);
    else if(p==='circle') this.placeCircle(cfg.rows);
    else if(p==='box') this.placeHollowBox(cfg.rows,cfg.cols);
  }
  spawnAlien(gridX,gridY,totalRows){
    ...
    const rev=totalRows-1-gridY; const typeIdx=Math.min(this.alienTypes.length-1,Math.floor(rev/2)); const t=this.alienTypes[typeIdx];
    this.enemies.push({x,y,width:40,height:40,alive:true,hp:t.hp,maxHp:t.hp,type:typeIdx,oscPhase:Math.random()*Math.PI*2});
  }
  placeGrid(rows,cols){ for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++) this.spawnAlien(c,r,rows);} }
  placeTriangle(rows){ for(let r=0;r<rows;r++){ const cols=r+1; const start=-Math.floor(cols/2); for(let c=0;c<cols;c++) this.spawnAlien(start+c+Math.floor(rows/2),r,rows);} }
  placeCircle(d){ const r=d/2; for(let gy=-r;gy<=r;gy++){ for(let gx=-r;gx<=r;gx++){ if(gx*gx+gy*gy<=r*r) this.spawnAlien(gx+r,gy+r,d);} } }
  placeHollowBox(rows,cols){ for(let c=0;c<cols;c++){ this.spawnAlien(c,0,rows); this.spawnAlien(c,rows-1,rows);} for(let r=1;r<rows-1;r++){ this.spawnAlien(0,r,rows); this.spawnAlien(cols-1,r,rows);} }
  ...
  /* ------------ Cracks (as v8) ------------- */
  drawZigZagCrack(x,y,w,h){ const seg=6; const step=w/seg; let cx=x, cy=y+Math.random()*h; this.ctx.beginPath(); this.ctx.moveTo(cx,cy); for(let i=0;i<seg;i++){ cx+=step; cy+= (Math.random()-0.5)*h*0.5; cy=Math.max(y,Math.min(y+h,cy)); this.ctx.lineTo(cx,cy);} this.ctx.stroke(); }
  drawCracks(x,y,w,h,sev){ const m=4; sev=Math.min(sev,3); const use=this.imgReady(this.crackImgs[0]); if(use){ for(let i=0;i<sev;i++) this.ctx.drawImage(this.crackImgs[i],x+m,y+m,w-2*m,h-2*m);} else { this.ctx.strokeStyle='rgba(255,255,255,0.4)'; this.ctx.lineWidth=2; let n=sev; while(n--) this.drawZigZagCrack(x+m,y+m,w-2*m,h-2*m);} }
  ...
  handleCollisions(){
    this.playerBullets.forEach((b,bi)=>{ this.enemies.forEach(e=>{ if(!e.alive) return; if(b.x<e.x+e.width && b.x+b.width>e.x && b.y<e.y+e.height && b.y+b.height>e.y){ e.hp--; this.playerBullets.splice(bi,1); if(e.hp<=0){ e.alive=false; this.score+=this.alienTypes[e.type].score; this.playSound('explosion'); } } }); });
  ...
  /* ------------ Drawing ------------- */
  drawBackground(){ if(this.imgReady(this.bgImg)) this.ctx.drawImage(this.bgImg,0,this.hudHeight,this.canvas.width,this.canvas.height-this.hudHeight); }
  ...
  drawHud(){ const cfg=this.levels[this.currentLevel];
    ...
    this.ctx.fillText('HS:'+this.getHighScore(this.currentLevel), this.canvas.width-140,50);
    // miniature
    const img=cfg.imgObj; if(this.imgReady(img)){ const maxH=this.hudHeight-14; const ratio=img.naturalWidth/img.naturalHeight; const h=maxH; const w=h*ratio; const x=this.canvas.width-w-10; const y=12; this.ctx.drawImage(img,x,y,w,h); this.ctx.strokeStyle='#fff'; this.ctx.strokeRect(x,y,w,h); }
  }
  ...
  drawStart(){ const cfg=this.levels[this.selectedLevel];
    ...
    const img=cfg.imgObj; if(this.imgReady(img)){ const maxH=120; const ratio=img.naturalWidth/img.naturalHeight; const h=maxH; const w=h*ratio; const x=this.canvas.width/2-w/2; const y=this.canvas.height-h-20; this.ctx.drawImage(img,x,y,w,h); this.ctx.strokeStyle='#fff'; this.ctx.strokeRect(x,y,w,h); }
  }
