// Menel Invaders – v11
// Fix Game-Over restart bug, add Praga (heart) level and power-ups (burger, coffee, pizza)
...
class MenelInvaders {
  constructor() {
    ...
    this.levels = [
      ...
      { name: 'Praga',              pattern: 'heart', rows: 12, cols: 12, speed: 1.2,  photo: 'assets/praga.jpg' }
    ];
    ...
    this.powerUpImgs.life.src   = 'assets/burger.png';
    this.powerUpImgs.rapid.src  = 'assets/coffee.png';
    this.powerUpImgs.spread.src = 'assets/pizza.png';
  ...
  /* ===== Helper ===== */
  imgReady(i){return i.complete&&i.naturalWidth;}

  /* ===== Stars (parallax as v10) ===== */
  ...
  setupControls(){
    window.addEventListener('keydown',e=>{
      ...
      if(k===' '){
        if(this.state==='start'){this.score=0; this.loadLevel(this.selectedLevel); this.state='playing';}
  ...
  /* ===== Level handling ===== */
  resetPlayer(){this.player.lives=this.player.maxLives; this.player.velX=0; this.shootCooldown=this.shootCooldownBase; this.spreadFire=false;}
  ...
  initPattern(cfg){
    switch(cfg.pattern){
      case 'grid': this.placeGrid(cfg.rows,cfg.cols); break;
      case 'triangle': this.placeTriangle(cfg.rows); break;
      case 'circle': this.placeCircle(cfg.rows); break;
      case 'box': this.placeHollowBox(cfg.rows,cfg.cols); break;
      case 'heart': this.placeHeart(cfg.rows,cfg.cols); break;
    }
  }

  spawnAlien(gx,gy,totalRows){const x=this.enemyOffsetX+gx*this.enemySpacingX,y=this.enemyOffsetY+gy*this.enemySpacingY;const rev=totalRows-1-gy;const tIdx=Math.min(this.alienTypes.length-1,Math.floor(rev/2));const t=this.alienTypes[tIdx];this.enemies.push({x,y,width:40,height:40,alive:true,hp:t.hp,maxHp:t.hp,type:tIdx,oscPhase:Math.random()*Math.PI*2});}
  ...
  placeHeart(rows,cols){const h=rows;const w=cols;for(let gy=0;gy<h;gy++){for(let gx=0;gx<w;gx++){const nx=(gx-w/2)/(w/2),ny=(gy-h/2)/(h/2);const eq=Math.pow(nx*nx+ny*ny-0.3,3)-nx*nx*ny*ny*ny; if(eq<=0) this.spawnAlien(gx,gy,h);}}}

  /* ===== Shooting & Power timers ===== */
  tryShoot(){const now=performance.now();if(now-this.lastShot<this.shootCooldown)return;this.lastShot=now;this.shootPlayer();}

  shootPlayer(){if(this.spreadFire){this.playerBullets.push({x:this.player.x+this.player.width/2-2,y:this.player.y,dx:-1,speed:9});this.playerBullets.push({x:this.player.x+this.player.width/2-2,y:this.player.y,dx:0,speed:9});this.playerBullets.push({x:this.player.x+this.player.width/2-2,y:this.player.y,dx:1,speed:9});}else{this.playerBullets.push({x:this.player.x+this.player.width/2-2,y:this.player.y,dx:0,speed:9});}this.playSound('playerShoot');}
  ...
  /* ===== PowerUps ===== */
  maybeSpawnPowerUp(x,y){if(Math.random()<0.15){const types=['life','rapid','spread'];const type=types[Math.floor(Math.random()*types.length)];this.powerUps.push({x,y,width:24,height:24,type,speed:2});}}
  applyPowerUp(p){if(p.type==='life'){this.player.lives=Math.min(this.player.maxLives,this.player.lives+1);}else if(p.type==='rapid'){this.shootCooldown=this.shootCooldownBase/2;this.powerTimers.rapid=performance.now()+10000;}else if(p.type==='spread'){this.spreadFire=true;this.powerTimers.spread=performance.now()+10000;}
  ...
  update(){this.updateStars();this.updatePowerTimers();
    if(this.leftPressed) this.player.velX-=this.player.acc; if(this.rightPressed) this.player.velX+=this.player.acc; this.player.velX*=this.player.friction; this.player.velX=Math.max(-this.player.maxSpeed,Math.min(this.player.maxSpeed,this.player.velX));
    ...
    if(this.enemies.every(e=>!e.alive)){this.saveHighScore();this.loadLevel((this.currentLevel+1)%this.levels.length);} if(this.player.lives<=0){this.saveHighScore();this.state='gameover';this.playSound('explosion');}
  }
  ...
  /* ===== Draw helpers ===== */
  drawPowerUps(){this.powerUps.forEach(p=>{const img=this.powerUpImgs[p.type]; if(this.imgReady(img)) this.ctx.drawImage(img,p.x,p.y,p.width,p.height); else {this.ctx.fillStyle='#0ff'; this.ctx.fillRect(p.x,p.y,p.width,p.height);}});}
  ...
  drawHud(){const cfg=this.levels[this.currentLevel];this.ctx.fillStyle='#d48b07';this.ctx.fillRect(0,0,this.canvas.width,this.hudHeight);this.ctx.fillStyle='#fff';this.ctx.fillRect(0,0,this.canvas.width,10);
    this.ctx.fillStyle='#fff';this.ctx.font='18px "Comic Sans",Arial';this.ctx.textAlign='left';this.ctx.fillText('🍺 '+cfg.name,10,30);this.ctx.fillText('Lives:'+this.player.lives,10,50);
  ...
  drawEntities(ts){const time=ts||performance.now(); // stars handled separately
    // player
  ...
  drawStart(){const cfg=this.levels[this.selectedLevel];this.ctx.fillStyle='#000';this.ctx.fillRect(0,0,this.canvas.width,this.canvas.height);this.drawStars();this.ctx.fillStyle='#fff';this.ctx.font='48px Arial';this.ctx.textAlign='center';this.ctx.fillText('MENEL INVADERS',this.canvas.width/2,this.canvas.height/2-100);this.ctx.font='24px Arial';this.ctx.fillText('Choose City (←/→) – '+cfg.name,this.canvas.width/2,this.canvas.height/2-40);this.ctx.fillText('Highscore:'+this.getHighScore(this.selectedLevel),this.canvas.width/2,this.canvas.height/2-10);this.ctx.fillText('Press SPACE to Start',this.canvas.width/2,this.canvas.height/2+30);const img=cfg.imgObj;if(this.imgReady(img)){const maxH=120,ratio=img.naturalWidth/img.naturalHeight,h=maxH,w=h*ratio,x=this.canvas.width/2-w/2,y=this.canvas.height-h-20;this.ctx.drawImage(img,x,y,w,h);this.ctx.strokeStyle='#fff';this.ctx.strokeRect(x,y,w,h);}this.ctx.textAlign='left';}
  ...
  /* ===== HS util ===== */
  saveHighScore(){const key='menelHS_'+this.levels[this.currentLevel].name;const prev=parseInt(localStorage.getItem(key)||'0');if(this.score>prev)localStorage.setItem(key,this.score);}getHighScore(i){return localStorage.getItem('menelHS_'+this.levels[i].name)||0;}
}
